...
def succ(state, player):  # (Up/Down , Right/Left)
    ...
    for dir in get_directions():
        ...
        if 0 <= newloc[0] < len(state.current_board):
            if 0 <= newloc[1] < len(state.current_board[0]):
                if state.current_board[newloc[0]][newloc[1]] == 0 or state.current_board[newloc[0]][newloc[1]] > 2:
                    moves.append(dir)
    return moves

def heuristic(state):
    sum_scores = (state.scores[0]+state.scores[1])
    if (sum_scores == 0):
        return 0
    else:
        return state.scores[0]/sum_scores
...
def preformMove(state, player, move):
    ...
    if state.current_board[newloc[0]][newloc[1]] > 0 and state.turns_played <= state.fruit_turn_timer:
...
def undoMove(state, player, move):
    ...
    if currloc in state.removed_fruits:
        ...
        if state.turns_played <= state.fruit_turn_timer:
